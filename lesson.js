...
completeButton.addEventListener("click", () => {
  ...
  const wasAlreadyCompleted = (currentState.completedLessons || 0) >= lessonNumber;
  ...
  if (!wasAlreadyCompleted) {
    currentState.xp = Math.max(0, Number(currentState.xp) || 0) + LESSON_XP_REWARD;
  }
  ...
  status.textContent =
    lessonNumber >= TOTAL_LESSONS
      ? `Mission ${chapterLessonNumber} du chapitre ${chapterNumber} validee. Le parcours est termine.`
      : chapterLessonNumber >= CHAPTER_SIZE
